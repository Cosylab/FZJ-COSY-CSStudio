...
public final class SymmetricAxis extends ValueAxis<Number> {
    ...
    private DoubleProperty tickUnit = new StyleableDoubleProperty(5) {
        ...
        @Override
        public CssMetaData<SymmetricAxis,Number> getCssMetaData() {
     ...
     */
    public SymmetricAxis(double lowerBound, double upperBound, double tickUnit) {
     ...
     * @param tickUnit The tick unit, ie space between tickmarks
     */
    public SymmetricAxis(String axisLabel, double lowerBound, double upperBound, double tickUnit) {
        super(lowerBound,upperBound);
        setTickUnit(tickUnit);
        setLabel(axisLabel);
    }
    ...
    protected List<Number> calculateMinorTickMarks() {
        ...
        if (tickUnit > 0) {
            if (((upperBound - lowerBound) / minorUnit) > 10000) {
                // This is a ridiculous amount of major tick marks, something has probably gone wrong
                System.err.println(
                        "Warning we tried to create more than 10000 minor tick marks on a NumberAxis. " + "Lower Bound="
                                + getLowerBound() + ", Upper Bound=" + getUpperBound() + ", Tick Unit=" + tickUnit);
    ...
    private static class StyleableProperties {
        ...
        private static final CssMetaData<SymmetricAxis,Number> TICK_UNIT = new CssMetaData<SymmetricAxis,Number>(
                "-fx-tick-unit",SizeConverter.getInstance(),5.0) {

            @Override
            public boolean isSettable(SymmetricAxis n) {
                return n.tickUnit == null || !n.tickUnit.isBound();
            }

            @Override
            public StyleableProperty<Number> getStyleableProperty(SymmetricAxis n) {
    ...
    public static class DefaultFormatter extends StringConverter<Number> {
         ...
         */
        public DefaultFormatter(final SymmetricAxis axis) {
         ...
         */
        public DefaultFormatter(SymmetricAxis axis, String prefix, String suffix) {
            this(axis);
            this.prefix = prefix;
            this.suffix = suffix;
        }
