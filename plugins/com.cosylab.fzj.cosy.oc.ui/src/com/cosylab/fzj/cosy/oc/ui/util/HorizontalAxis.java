 ...
 * @since JavaFX 2.0
 *
 * This is a copy of javafx.scene.chart.NumberAxis, with special tick formatter, where ticks smaller than 0 are not
 *          displayed.
 */
public final class HorizontalAxis extends ValueAxis<Number> {
    ...
    private DoubleProperty tickUnit = new StyleableDoubleProperty(5) {
        ...
        @Override
        public CssMetaData<HorizontalAxis,Number> getCssMetaData() {
     ...
     */
    public HorizontalAxis(double lowerBound, double upperBound, double tickUnit) {
     ...
     * @param tickUnit The tick unit, ie space between tickmarks
     */
    public HorizontalAxis(String axisLabel, double lowerBound, double upperBound, double tickUnit) {
        super(lowerBound, upperBound);
        setTickUnit(tickUnit);
        setLabel(axisLabel);
    }
    ...
    private static class StyleableProperties {
        private static final CssMetaData<HorizontalAxis,Number> TICK_UNIT =
            new CssMetaData<HorizontalAxis,Number>("-fx-tick-unit",
                SizeConverter.getInstance(), 5.0) {

            @Override
            public boolean isSettable(HorizontalAxis n) {
                return n.tickUnit == null || !n.tickUnit.isBound();
            }

            @Override
            public StyleableProperty<Number> getStyleableProperty(HorizontalAxis n) {
    ...
    public static class DefaultFormatter extends StringConverter<Number> {
         ...
         */
        public DefaultFormatter(final HorizontalAxis axis) {
         ...
         */
        public DefaultFormatter(HorizontalAxis axis, String prefix, String suffix) {
            this(axis);
            this.prefix = prefix;
            this.suffix = suffix;
        }
